...
const projects = [
  ...
  { 
    ...
    image: "https://images.unsplash.com/photo-1543269865-cbf427effbad?q=80&w=2000&auto=format&fit=crop", // Reliable Editorial Image
    tags: ["Editorial", "2025"],
    aspect: "aspect-[21/9]"
  },
  { 
    title: "Botanical 3D", 
    image: "https://images.unsplash.com/photo-1614850523459-c2f4c699c52e?q=80&w=2000&auto=format&fit=crop", // Reliable Abstract 3D
    tags: ["3D Design", "2025"],
    aspect: "aspect-square"
  },
  { 
    title: "Mobile App Sol", 
    image: "https://images.unsplash.com/photo-1616469829941-c7200edec809?q=80&w=2000&auto=format&fit=crop",
...
const WorkCard = ({ project, className }) => {
    ...
    return (
        <div className={`group flex flex-col gap-6 ${className}`}>
            {/* Image Container */}
            <div ref={containerRef} className={`relative w-full overflow-hidden bg-neutral-900 ${project.aspect}`}>
                <Image 
                    ref={imgRef}
                    src={project.image} 
                    alt={project.title}
            ...
            <div className="flex flex-col gap-3">
                <h3 className="text-xl md:text-2xl font-bold text-white">Crafting Digital Experiences</h3>
  ...
  return (
     <section className="py-24 px-6 md:px-16 bg-[#0e0e0e] text-white overflow-hidden">
           ...
           <div className="flex flex-col gap-8 max-w-sm w-full md:w-auto self-start md:self-center">
               ...
               <div className="flex gap-4">
                  {["Dribbble", "Behance", "GitHub"].map((tag) => (
                      <span key={tag} className="px-4 py-2 bg-neutral-900 border border-zinc-800 rounded-full text-xs font-bold uppercase tracking-wider text-zinc-400 cursor-pointer hover:bg-white hover:text-black transition-colors">
                          {tag}
                      </span>
                  ))}
               </div>
        ...
        <div className="flex justify-center">
            <button className="group flex items-center gap-3 bg-[#d0fd3e] text-black px-8 py-4 rounded-full font-bold uppercase tracking-wider hover:bg-white transition-colors">
                View All Work
