      ...
      <header className="absolute top-0 left-0 w-full z-50 px-6 md:px-10 py-6 flex justify-between items-center text-white mix-blend-difference">
        ...
        <nav className="hidden xl:flex flex-1 justify-center">
          <ul className="flex items-center gap-8 text-xs font-bold uppercase tracking-widest">
            {["Home", "About", "Services", "Portfolio", "Blog"].map((item) => (
              <li
                key={item}
                className="cursor-pointer hover:text-neutral-400 transition-colors relative group"
              >
                <Link
                  href={
                    item === "Home"
        ...
        <div className="flex-1 flex justify-end items-center gap-4">
          ...
          <div className="hidden md:block">
            <MagneticButton>
              <Link href="/contact">
                <button className="bg-white text-black px-6 py-3 !rounded-none flex items-center gap-3 font-bold text-xs uppercase tracking-widest hover:bg-neutral-200 transition-colors shadow-none">
      ...
      >
        ...
        <div className="mt-12 md:hidden">
          <Link href="/contact" onClick={() => setIsMenuOpen(false)}>
            <button className="bg-white text-black px-8 py-4 !rounded-none flex items-center gap-3 font-bold uppercase tracking-widest hover:bg-neutral-200 transition-colors shadow-none">
              Let's Talk
            </button>
          </Link>
        </div>
