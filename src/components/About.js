...
export default function About() {
  ...
  return (
    <section
      id="about"
      ref={container}
      className="py-20 px-6 md:px-10 bg-[#0e0e0e] text-white overflow-hidden relative z-20"
    >
      ...
      <div className="mb-8">
        <h2 className="text-[15px] md:text-[63px] font-black uppercase tracking-tighter leading-[1.1] md:leading-[0.9] w-full">
          <span className="about-title-line flex items-end gap-3 md:gap-5">
            <span className="w-12 md:w-20 h-[2px] bg-neutral-600 mb-3 md:mb-5"></span>
            We're Friendly And{" "}
            <span className="text-neutral-600">Experienced</span>
          </span>
          <span className="about-title-line block">
            Creative Agency Based In California
          </span>
          <span className="about-title-line block">
            And Our Team Creates An Exceptional
          </span>
          <span className="about-title-line block">
            UI/UX Visualization And{" "}
            <span className="text-neutral-600">Thought-Out</span>
          </span>
          <span className="about-title-line block">
            Functionality By Thinking Creative
          </span>
      ...
      <div className="flex flex-col items-center justify-center text-center mb-10 md:mb-16">
        <p className="about-desc text-sm md:text-base leading-relaxed text-gray-400 max-w-2xl mb-6 text-center mx-auto">
          ...
          Build, Ship And Scale Your Vision In The Most Efficient Way. We
          Believe That The Surest Measure Of Success Is When A Client Partners
          With Us More Than Once.
        </p>
        <a
          href="#"
          className="about-link group flex items-center gap-2 text-xs font-bold uppercase tracking-widest pb-1 border-b border-white hover:text-gray-300 hover:border-gray-300 transition-colors"
        >
          Know More Us
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-10 border-t border-white/10 pt-16">
        {stats.map((stat, i) => (
          <div key={i} className="stat-item flex items-center gap-4">
            ...
            <span className="text-[10px] md:text-xs font-bold uppercase leading-tight text-white">
              {stat.label}
            </span>
          </div>
        ))}
